...
class FirstSchurCompactModelAction:
    """Ordered diagonal action plus one annotated Schur-correction integral."""

    relation: ModCompactSchurRelation
    diagonal: AppliedLinearCombination
    correction: KernelAnnotatedExpression
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.correction, KernelAnnotatedExpression):
            raise TypeError("correction must be a KernelAnnotatedExpression.")
        if not isinstance(self.correction.expression, sp.Integral):
            raise TypeError("correction.expression must be a SymPy Integral.")

    def as_expr(self) -> sp.Expr:
        """Project the separated model action to its final scalar expression."""

        return self.diagonal.as_expr() + self.correction.expression
...
def apply_a22_first_schur_model_compact(
    ...
    *,
    ...
    The kernel-correction layer requires an undefined scalar function applied
    directly to the external variable and an explicit ``regularizer_kernel``.
    The returned modulo-compact relation remains a semantic declaration; this
    function does not prove compactness. General scalar operands are
    intentionally outside the scope of this m=2 API.
    """
